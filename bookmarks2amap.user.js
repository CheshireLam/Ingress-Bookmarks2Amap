    ...
    function runExport() {
      ...
      setTimeout(function () {
        ...
        function renderHints() {
          if (!hintsEl) return;
          hintsEl.innerHTML = [
            '<p style="white-space:normal;word-break:break-word;margin:0;line-height:1.4;">1) 高德地图、腾讯地图、百度地图均支持选择起始 Portal、目的 Portal。</p>',
            '<p style="white-space:normal;word-break:break-word;margin:0;line-height:1.4;">2) 高德地图支持多选途经 Portal，移动端最多 16 个，网页端最多 6 个。</p>',
            '<p style="white-space:normal;word-break:break-word;margin:0;line-height:1.4;">3) 如果无法跳转，请复制并使用系统浏览器打开链接。</p>'
          ].join('');
        }
        ...
        refreshOutputs();
        // iOS Safari may auto-open the first <select>; proactively blur controls after initial render.
        setTimeout(function () {
          try {
            if (mapProviderSelect) mapProviderSelect.blur();
            if (startSelect) startSelect.blur();
            if (endSelect) endSelect.blur();
